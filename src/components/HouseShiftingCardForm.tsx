...
const BHK_OPTIONS = [
  { label: '1 RK', tag: 'Studio' },
  { label: '1 BHK', tag: 'Standard' },
  { label: '2 BHK', tag: 'Popular' },
  { label: '3 BHK', tag: 'Spacious' },
  { label: '4+ BHK', tag: 'Villa' },
];

interface HouseShiftingCardFormProps {
  selectedCity: string;
...
export const HouseShiftingCardForm: React.FC<HouseShiftingCardFormProps> = ({
  selectedCity,
  onSelectCity,
  onOpenCityModal,
  onOpenLoginModal,
  ...
  // 3. BHK House size selection
  const [houseSize, setHouseSize] = useState<string>('2 BHK');

  // 4. Pickup & Drop Locations
  const [pickupLoc, setPickupLoc] = useState<string>('');
  const [dropLoc, setDropLoc] = useState<string>('');

  // 5. Moving Date & Flexibility
  const [shiftingDate, setShiftingDate] = useState<string>(() => {
  ...
  const calculatedBasePrice = relocateType === 'between-city' ? 9500 : basePrice;

  const handleGetEstimate = () => {
    // Save selected form state to localStorage for seamless calculation
    const shiftingData = {
      relocateType,
    ...
    localStorage.setItem('ps_house_shifting_draft', JSON.stringify(shiftingData));
  ...
  return (
    <div id="house-shifting-form" className="bg-white dark:bg-slate-900 rounded-2xl sm:rounded-3xl border border-slate-200/90 dark:border-slate-800 shadow-xl overflow-visible relative p-3.5 sm:p-4.5 space-y-2.5 sm:space-y-3 text-left transition-all w-full max-w-lg sm:max-w-xl mx-auto lg:max-w-none scroll-mt-24">
      ...
      {/* 1. TOGGLE: Where are you transporting goods? (Within City / Between City) */}
      ...
      {/* 3. BHK / HOUSE SIZE SELECTOR */}
      <div className="space-y-1">
        <label className="text-[10px] font-extrabold uppercase tracking-wider text-slate-500 dark:text-slate-400 block">
          Select House Size (BHK)
        </label>
        
        <div className="grid grid-cols-5 gap-1">
          {BHK_OPTIONS.map((opt) => {
            const isSelected = houseSize === opt.label;
            return (
              <button
                key={opt.label}
                type="button"
                onClick={() => setHouseSize(opt.label)}
                className={`py-1.5 px-1 rounded-xl text-center transition-all cursor-pointer border ${
                  isSelected
                    ...
                    : 'bg-slate-50 dark:bg-slate-950 text-slate-700 dark:text-slate-300 border-slate-200/80 dark:border-slate-800 hover:border-blue-400'
                }`}
              >
                <div className="text-[11px] font-black leading-none mb-0.5">{opt.label}</div>
                <div className={`text-[8px] font-medium leading-none ${isSelected ? 'text-blue-200' : 'text-slate-400'}`}>
                  {opt.tag}
                </div>
              </button>
            );
      ...
      {/* 4. PICKUP AND DROP LOCATION TEXTBOXES WITH GOOGLE MAP CONNECTOR */}
      <div className="space-y-1">
        <label className="text-[10px] font-extrabold uppercase tracking-wider text-slate-500 dark:text-slate-400 block">
          Pickup &amp; Drop Address
        </label>
      ...
      {/* 5. MOVING DATE SELECTION (For between-city or date planning) */}
      {relocateType === 'between-city' && (
        <div className="space-y-1">
          <div className="flex items-center justify-between">
            ...
            <label className="flex items-center gap-1 cursor-pointer text-[10px] text-slate-600 dark:text-slate-400 font-semibold select-none">
              <input
                type="checkbox"
                checked={isFlexibleDate}
                onChange={(e) => setIsFlexibleDate(e.target.checked)}
                className="w-3 h-3 text-[#001261] rounded border-slate-300 focus:ring-[#001261]"
              />
              <span>Flexible (+/- 2 days)</span>
            </label>
          </div>
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200/80 dark:border-slate-800 rounded-xl pl-8 pr-2.5 py-1.5 text-xs font-bold text-slate-900 dark:text-white outline-none focus:border-blue-600 dark:focus:border-blue-500 transition-all shadow-2xs"
            />
          </div>
        </div>
      )}

      {/* 6. GET ESTIMATE BUTTON */}
      <div className="pt-1 space-y-1.5">
        ...
        <p className="text-[9px] text-center text-slate-400 dark:text-slate-500 font-semibold">
          ⚡ 100% Free Quote • No Advance Needed • Verified Packers
        </p>
      </div>
