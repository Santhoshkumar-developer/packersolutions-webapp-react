...
interface LoadingUnloadingCardFormProps {
  selectedCity: string;
...
export const LoadingUnloadingCardForm: React.FC<LoadingUnloadingCardFormProps> = ({
  selectedCity,
  onSelectCity,
  onOpenCityModal,
  onOpenLoginModal,
  ...
  // 2. Loading Date & Time state
  const [loadingDate, setLoadingDate] = useState<string>(() => {
    const nextDate = new Date();
    nextDate.setDate(nextDate.getDate() + 1);
    return nextDate.toISOString().split('T')[0];
  });
  const [loadingTimeSlot, setLoadingTimeSlot] = useState<string>(TIME_SLOTS[0]);
  ...
  const handleGetEstimate = () => {
    ...
    const loadingBookingData = {
      bookingType: 'loading-unloading',
      serviceName,
      city: selectedCity,
    ...
    localStorage.setItem('ps_loading_booking_draft', JSON.stringify(loadingBookingData));
  ...
  return (
    <div id="loading-unloading-form" className="bg-white dark:bg-slate-900 rounded-2xl sm:rounded-3xl border border-slate-200/90 dark:border-slate-800 shadow-xl overflow-visible relative p-3.5 sm:p-4.5 space-y-2.5 sm:space-y-3 text-left transition-all w-full max-w-lg sm:max-w-xl mx-auto lg:max-w-none scroll-mt-24">
      ...
      <div className="flex items-center justify-between pt-0.5">
        <div className="space-y-0.5">
          <div className="inline-flex items-center gap-1 bg-blue-50 dark:bg-blue-950/70 border border-blue-200 dark:border-blue-800/80 px-2 py-0.5 rounded-full text-[9px] font-extrabold text-[#001261] dark:text-blue-300">
            ...
            <span>Certified Heavy Lifters</span>
          </div>
      ...
      <div className="pt-1 space-y-1.5">
        ...
        >
          <span>Calculate Loading Fare &amp; Book</span>
        ...
        <p className="text-[9px] text-center text-slate-400 dark:text-slate-500 font-semibold flex items-center justify-center gap-1">
          ...
          <span>Heavy Equipment Ready • Zero Damage Guarantee • Verified Helpers</span>
        </p>
      </div>
