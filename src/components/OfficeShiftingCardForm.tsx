...
export interface OfficeAreaOption {
  id: string;
  approxArea: string;
  desks: string;
...
export const OFFICE_AREA_OPTIONS: OfficeAreaOption[] = [
  {
    id: 'area-300',
    approxArea: 'Up to 500 Sq.ft',
    desks: '1-10 Desks',
    startingPrice: 8999,
    tag: 'Compact'
  },
...
interface OfficeShiftingCardFormProps {
  selectedCity: string;
...
export const OfficeShiftingCardForm: React.FC<OfficeShiftingCardFormProps> = ({
  selectedCity,
  onSelectCity,
  onOpenCityModal,
  onOpenLoginModal,
  onOpenEnquiryModal,
  serviceName = "Office & Corporate Shifting",
  basePrice = 8999
}) => {
  ...
  // 2. Cities & Search dropdown states
  ...
  const calculateDisplayPrice = () => {
    if (relocateType === 'between-city') {
      return `₹${(basePrice + 12000).toLocaleString()}`;
    }
  ...
  const handleGetEstimate = () => {
    const shiftingData = {
      serviceType: 'office-corporate-shifting',
      relocateType,
      originCity: selectedCity,
      destinationCity: relocateType === 'between-city' ? destinationCity : selectedCity,
      pickupAddress: pickupAddress || `${selectedCity} Commercial Hub`,
      dropAddress: dropAddress || (relocateType === 'between-city' ? `${destinationCity} Tech Park` : `${selectedCity} Tech Park`),
      approxArea: 'Commercial Office Space',
      desks: 'Standard Office Scale',
      estimatedStartingPrice: calculateDisplayPrice()
    };
    
    localStorage.setItem('ps_office_shifting_draft', JSON.stringify(shiftingData));
    localStorage.setItem('ps_house_shifting_draft', JSON.stringify({
      relocateType,
      originCity: selectedCity,
      destinationCity: relocateType === 'between-city' ? destinationCity : selectedCity,
      houseSize: 'Commercial Office Space',
      pickupLoc: pickupAddress,
      dropLoc: dropAddress
    }));
  ...
  return (
    <div id="office-shifting-form" className="bg-white dark:bg-slate-900 rounded-2xl sm:rounded-3xl border border-slate-200/90 dark:border-slate-800 shadow-xl overflow-visible relative p-3.5 sm:p-4.5 space-y-2.5 sm:space-y-3 text-left transition-all w-full max-w-lg sm:max-w-xl mx-auto lg:max-w-none scroll-mt-24">
      ...
      {/* 2. CITY SELECTION DROPDOWN (With searchable dropdown) */}
      <div className="space-y-1">
        <label className="text-[10px] font-extrabold uppercase tracking-wider text-slate-500 dark:text-slate-400 block">
      ...
      <div className="pt-1.5 border-t border-slate-100 dark:border-slate-800 grid grid-cols-2 gap-2 text-[9px] font-bold text-slate-600 dark:text-slate-400">
        ...
        <div className="flex items-center gap-1">
          ...
          <span>GST B2B Tax Invoicing</span>
        </div>
      </div>
